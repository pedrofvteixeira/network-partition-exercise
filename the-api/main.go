...
func postOperation(ctx *gin.Context) {
	...
	_, op = store.Create(op.Name)

	go updateOperationState(op)

	_, op = store.ReadById(op.Id)
	ctx.JSON(http.StatusOK, op)
}

func updateOperationState(op store.Operation) {
	...
	err := try.Do(func(attempt int) (bool, error) {
		...
		log.Printf("POSTing to %s with data %v", url, string(data))
		resp, err := http.Post(url, "application/json", bytes.NewBuffer(data))

		if err != nil {
			log.Println(err.Error())
			time.Sleep(time.Minute) // wait a minute
			return true /*should retry*/, err
		}
		...
		success := (resp.StatusCode >= http.StatusOK && resp.StatusCode < http.StatusMovedPermanently)

		if success {
			op.State = store.Succeeded
			op.UpstreamId = upstreamOp.Id
			store.Update(op)
			return false /*should retry*/, err
		}
